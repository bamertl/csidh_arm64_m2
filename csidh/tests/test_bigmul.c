...
void uint_to_hex_string(const uint* value, char* buffer, size_t buffer_size) {
    // Start from the end of the biguint array (most significant word)
    for (int i = 8 - 1; i >= 0; i--) {
        // Append each word in hexadecimal format to the buffer
        // snprintf prevents buffer overflow
...
void test_big_mul_example2(void)
{
    uint a = {{1,2,3,4,5,6,7,8}};
    uint b = {{1, 0, 0, 0, 0, 0, 0, 0}};
    printf("a = "); uint_print(&a);
    printf("b = "); uint_print(&b);
    biguint c = {{0}};
    uint_mul_512x512(&a, &b, &c);
    ...
    biguint_to_hex_string(&c, hex_str, sizeof(hex_str));
    printf("c = %s\n", hex_str);
    const char* expected_c_str = "";
    assert(strcmp(hex_str, expected_c_str) == 0);
}
...
void test_uint_mul_lower2(void)
{
    smalluint a = {{0x1a2b3c4d5e6f7081, 0x9a8b7c6d5e4f3021, 0x1234567890abcdef, 0xfedcba9876543210}};
    smalluint b = {{0x9988776655443322, 0x1122334455667788, 0x99aabbccddeeff00, 0x0011223344556677}};
    printf("a = "); small_uint_print(&a);
    printf("b = "); small_uint_print(&b);
    uint c = {{0,0,0,0,0,0,0,0}};
    uint_mul_lower(&a, &b, &c);
    char hex_str[512];
    uint_to_hex_string(&c, hex_str, sizeof(hex_str));
    printf("c = %s\n", hex_str);
    const char* expected_c_str = "00110eb4bee8ee8b8c7a596dfc47e9a209c34899b5f385bc1ca3ca455c82f2c4b8dae50a7e8698be95d45b6f550ae6dbed282cb68148c9d016e44c71757aa422";
    assert(strcmp(hex_str, expected_c_str) == 0); 
}

void test_uint_mul_lower3(void)
{
    smalluint a = {{0x0f1e2d3c4b5a6978, 0x8172635455766879, 0xabcdef0123456789, 0x0123456789abcdef}};
    smalluint b = {{0x1b81b90533c6c879, 0xc2721bf457aca835, 0x516730cc1f0b4f25, 0xa7aac6c567f35507}};
    printf("a = "); small_uint_print(&a);
    printf("b = "); small_uint_print(&b);
    uint c = {{0,0,0,0,0,0,0,0}};
    uint_mul_lower(&a, &b, &c);
    ...
    uint_to_hex_string(&c, hex_str, sizeof(hex_str));
    printf("c = %s\n", hex_str);
    const char* expected_c_str = "00bec494c9d9425e34c2f8d3dd446fe5dc5030d8ad443c8560473bdff842c64f37ed0caa46a6b6434f0a094c852a040de71d512fa765358a54b38ea6baf199b8";
    assert(strcmp(hex_str, expected_c_str) == 0); 
}
...
{
    test_uint_mul_lower();
    test_uint_mul_lower2();
    test_uint_mul_lower3();
    test_big_mul();
    test_big_mul_example1();
    test_big_mul_example2();
